...
// This part allows us to input the name they entered into the question line so it is customary for each alter. (specifically for showing who knows who. Allowing participants to more easily and reliably show each of th ties)
  ...
  if (($('input[name=age]:checked').val() == 1 && $('input#agetextInput').val() == "") || ($('input[name=age]:checked').length == 0) && !checked) {
    ...
    // Collect data before going on
    answers.q36A = $('input[name=age]:checked').val();
    if ($('input[name=age]:checked').val() == 1) answers.q36_A += ":" + $('input#agetextInput').val();
    var d = new Date();
    answers.q36AtimeStamp = (d - startTime) / 1000;
      // Collect data before going on
      ...
      if (($('input[name=gender]:checked').length == 0)  && !checked && !skipped) {
        ...
        // Collect data before going on
        if(!skipped) {
          answers.q36B = $('input[name=gender]:checked').val();
          var d = new Date();
          answers.q36BtimeStamp = (d - startTime) / 1000;
        } else {
          answers.q36B = "skipped";
        }
   ...
   if (($('input[name=nation]:checked').val() == 4 && $('input#nationtextInput').val() == "") || ($('input[name=nation]:checked').length == 0) && !checked) {
     ...
     // Collect data before going on
     answers.q36D = $('input[name=nation]:checked').val();
     if ($('input[name=nation]:checked').val() == 4) answers.q36_D += ":" + $('input#nationtextInput').val();
     var d = new Date();
     answers.q36DtimeStamp = (d - startTime) / 1000;
       // Collect data before going on
   ...
   if (($('input[name=education]:checked').length == 0)  && !checked && !skipped) {
     ...
     // Collect data before going on
     if(!skipped) {
       answers.q36F = $('input[name=education]:checked').val();
       var d = new Date();
       answers.q36FtimeStamp = (d - startTime) / 1000;
     } else {
       answers.q36F = "skipped";
     }
  ...
  document.getElementById("education_input").style.display = "none";
    ...
    if (($('input[name=lo19]:checked').length == 0 || $('input[name=lo20]:checked').length == 0) && !checked && !skipped) {
      ...
      var ex = document.getElementById("instagram_input");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
  ...
  } else if (currSlide == 27.5) {

      if (($('input[name=instagram]:checked').length == 0)  && !checked && !skipped) {
      promptNonresponse();
      checked = true;
      } else {
      // Collect data before going on
      if(!skipped) {
        answers.q46_5= $('input[name=instagram]:checked').val();
        var d = new Date();
        answers.q46_5timeStamp = (d - startTime) / 1000;
      } else {
        answers.q46_5 = "skipped";
      }
       document.getElementById("instagram_input").style.display = "none";
...
// Instagram Questions signified by the 'in(number)' Called from the index.php 2 questions per page
    ...
    if (($('input[name=ha13]:checked').length == 0 || $('input[name=ha14]:checked').length == 0) && !checked && !skipped) {
      ...
      var ex = document.getElementById("region_input");
      ex.style.left = string_l + "px";
      ex.style.top = string_t;
      ex.style.display = "block";
    ...
    if (($('input[name=region]:checked').length == 0)  && !checked && !skipped) {
      promptNonresponse();
      checked = true;
    } else {
      // Collect data before going on
      if(!skipped) {
        answers.q36E = $('input[name=region]:checked').val();
        var d = new Date();
        answers.q36EtimeStamp = (d - startTime) / 1000;
      } else {
        answers.q36E = "skipped";
      }
